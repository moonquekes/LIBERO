...
2) 输出 sorting_meta_r12_cf.json。
两级均衡逻辑与 r4 完全相同：布局内拉平 + P 层翻倍补形状全局配比。

用法：python scripts/mk_meta_r5.py   # 输出 $D/sorting_meta_r12_cf.json
"""
import glob
import json
import os
...
OUT = os.path.join(D, "sorting_meta_r12_cf.json")

EXCLUDE = {"round_blue_bin_selfc4", "round_blue_bin_selfc8b", "round_blue_bin_d9div"}

LAYOUT = {
    "rectangular_red_bin": "D", "rectangular_red_bin_selfc4": "D",
    "rectangular_red_bin_selfc8": "D", "rectangular_red_bin_selfc8b": "D",
    "round_blue_bin": "D", "round_blue_bin_selfc4": "D", "round_blue_bin_selfc8": "D", "round_blue_bin_selfc8b": "D", "round_blue_bin_d9div": "D",
    "triangular_yellow_bin": "D", "rectangular_red_bin_cf": "D", "round_blue_bin_cf": "D", "triangular_yellow_bin_cf": "D",
...
def main():
    ...
    for part in sorted(glob.glob(os.path.join(D, "xvla_meta_part_*.json"))):
        ...
        if dirname not in LAYOUT:
            print(f"SKIP 不在 r9(=r6+d9div) 配方的目录 {dirname}（r7/r8 残留）")
            continue
        with open(part) as f:
